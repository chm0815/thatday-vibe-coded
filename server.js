...
/**
 * Send (or log) a daily reminder email to a user.
 */
async function sendReminderEmail(user) {
  ...
  if (APP_ENV === "dev" || !mailTransport) {
    console.log(
      `\n[DEV] Daily reminder for ${user.email}: No entry for today yet!\n  Open ${appUrl}\n`
    );
    return;
  }
  ...
  await mailTransport.sendMail({
    from: SMTP_FROM,
    to: user.email,
    subject: "thatday — Don't forget today!",
    text: `Hi ${user.name},\n\nYou haven't captured today yet. One photo, one headline — that's all it takes.\n\n${appUrl}\n\n— thatday`,
    html: `<p>Hi ${user.name},</p><p>You haven't captured today yet. One photo, one headline — that's all it takes.</p><p><a href="${appUrl}">Open thatday</a></p><p>— thatday</p>`,
  });
}

/**
 * Check all confirmed users and send a reminder if they have no entry for today.
 */
async function checkAndSendReminders() {
  const today = getTodayInTimezone();
  console.log(`[Reminder] Checking for missing entries on ${today}...`);

  let userFiles;
  try {
    userFiles = fs.readdirSync(USERS_DIR).filter((f) => f.endsWith(".json"));
  } catch {
    console.error("[Reminder] Could not read users directory");
    return;
  }
  ...
  for (const file of userFiles) {
    try {
      ...
      console.error(`[Reminder] Error processing ${file}:`, err.message);
    }
  }
 ...
 * Checks every 60 seconds whether it's 17:00 in Europe/Berlin.
 * Uses a flag to avoid sending duplicates within the same minute.
 */
function startReminderScheduler() {
  ...
  setInterval(() => {
    ...
    const timeStr = formatter.format(now); // "17:00"
  ...
  console.log("[Reminder] Scheduler started — reminders at 17:00 Europe/Berlin");
}
